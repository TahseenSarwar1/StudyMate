...
import { leaderboardData, friendsLeaderboard } from '../data/mockData';
import { Trophy, Shield, Award, Sparkles, ChevronUp, ChevronDown, Flame, Timer } from 'lucide-react';
import { motion } from 'framer-motion';

export default function Leaderboard() {
  const [activeTab, setActiveTab] = useState('global'); // global | friends

  const currentLeaderboard = activeTab === 'global' ? leaderboardData : friendsLeaderboard;
        ...
        <main className="flex-1 space-y-8 min-w-0">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 w-fit text-[10px] font-black uppercase tracking-wider">
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg transition-all ${
                activeTab === 'friends' ? 'bg-white text-primary-650 shadow-sm' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              Friends List
            </button>
          ...
          <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto items-end pt-12 select-none h-60">
            {podium.map((pod, idx) => {
              ...
              // Arrange second, first, third columns
